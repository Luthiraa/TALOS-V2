...
counter_jtag_vjtag counter_jtag_vjtag_inst (
    .count_in(count_reg)
);
...
module counter_jtag_vjtag (
    input  wire [31:0] count_in
);

localparam [1:0] IR_SAMPLE = 2'b01;
...
always @(posedge tck) begin
    if ((ir_in == IR_SAMPLE) && virtual_state_cdr) begin
        shift_reg <= count_in;
    end else if ((ir_in == IR_SAMPLE) && virtual_state_sdr) begin
        shift_reg <= {tdi, shift_reg[31:1]};
    end
end

sld_virtual_jtag counter_reader_vjtag (
    .tck(tck),
    .tdi(tdi),
    .ir_in(ir_in),
    .tdo(tdo),
...
defparam counter_reader_vjtag.sld_auto_instance_index = "NO";
defparam counter_reader_vjtag.sld_instance_index = 0;
defparam counter_reader_vjtag.sld_ir_width = 2;
